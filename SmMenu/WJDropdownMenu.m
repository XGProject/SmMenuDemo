...
/num-10, self.menuBaseHeight/2);
    ...
    for (int i = 0; i < num; i++) {
        UILabel *lineLb = [[UILabel alloc]initWithFrame:CGRectMake((btnW+1)*i+btnW, self.menuBaseHeight/5, 1, self.menuBaseHeight/10*6.5)];
...
- (void)showFirstAndSecondTableView:(NSInteger)index{
    ...
    if (self.firstTableViewShow == NO) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            /**
             cell_h*self.dataSourceFirst.count
             会根据数据的多少行 显示tab的高
             ，在数据量少的时候可以用，
             如果数据过多会超出屏幕
             */
            if (cell_h*self.dataSourceFirst.count<300) {
                ...
                self.tableFirst.frame = CGRectMake(0, CGRectGetMaxY(self.backView.frame), self.tableViewWith, 300);
            }
...
- (void)showSecondTabelView:(BOOL)secondTableViewShow{
    if (self.secondTableViewShow == YES) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (cell_h*self.dataSourceSecond.count<300) {
...
#pragma mark -- TableViewFirst的属性设置
- (void)createTableViewFirst{
    ...
    self.tableFirst.separatorStyle=NO;
...
#pragma mark -- TableViewSecond的属性设置
...
/2, 0) style:UITableViewStylePlain];
    ...
    self.tableSecond.separatorStyle=NO;
    self.tableSecond.showsVerticalScrollIndicator=NO;
