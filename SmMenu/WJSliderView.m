...
@property (nonatomic,strong)NSArray *itemArray;//我的补充
...
- (void)commonInitWithArray:(NSArray<UIView *> *)array {
    ...
    [array enumerateObjectsUsingBlock:^(UIView * _Nonnull view, NSUInteger idx, BOOL * _Nonnull stop) {
        /*这里是调整label的位置*/
        view.frame = CGRectMake(idx * self.width, 10, self.width, self.height-3);
...
- (void)setIndexProgress:(CGFloat)indexProgress {
    ...
    for (int i=0; i<_itemArray.count; i++) {
        UILabel *lab =(UILabel *) [_itemArray objectAtIndex:i];
        [lab setTextColor:[UIColor blackColor]];
        if (i==(int) indexProgress) {
